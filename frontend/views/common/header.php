...
<!--                一级分类-->
                <?php foreach (\backend\models\Category::find()->where(['prent_id'=>0])->all() as $k1=>$v1):?>
                    <div class="cat <?=$k1==0?"item1":""?>">
                        <h3><a href="<?=\yii\helpers\Url::to(['list','id'=>$v1->id])?>"><?=$v1->name?></a> <b></b></h3>
                        <div class="cat_detail">
                            <!--                        二级分类-->
                            <?php foreach (\backend\models\Category::find()->where(['prent_id'=>$v1->id])->all() as $k2=>$v2){?>
                            <dl class="<?=$k2==0?"dl_1st":""?>">
                                <dt><a href="<?=\yii\helpers\Url::to(['list','id'=>$v2->id])?>"><?=$v2->name?></a></dt>
                                <dd>
                                    <!--                                三级分类-->
                                    <?php foreach (\backend\models\Category::find()->where(['prent_id'=>$v2->id])->all() as $k3=>$v3):?>
